...
function play_fill(){
  ...
  const next=()=>{
    ...
    document.getElementById('fo').onclick=e=>{
      ...
      if(b.dataset.w===s.word){ok++;addScore(40);}else wrongAnswer();
  ...
  const render=()=>{
    ...
    const sub=()=>{const v=document.getElementById('li').value.trim().toUpperCase();
      ...
      if(v===steps[aStep]){usr[aStep]=v;addScore(30);aStep++;
        ...
        render();}else{toast('❌ Doğru kelime değil!');wrongAnswer();}};
...
function play_blitz(){
  ...
  const tick=()=>{
    tl--;
    const tEl=document.getElementById('bz_tl');if(tEl)tEl.textContent=tl+'s';
    const tb=document.getElementById('bz_tb');if(tb)tb.style.width=(tl/75*100)+'%';
    if(tl<=0){
      clearInterval(gameTimer);
      showResults(sc>=150?'🏆':sc>=80?'🎲':'📝',sc>=150?'Blitz Kralı!':sc>=80?'İyi Tur!':'Isınma Turu!',sc+' puan',`${rd} soru tamamlandı`,'blitz');
    }
  };
  ...
  function nx(){
    ...
    let h=`<div class="card"><div class="card-t">🎲 Hızlı Tur</div>
      <div class="tbar"><div class="tfill" id="bz_tb" style="width:${tl/75*100}%"></div></div>
      <div class="gh"><div class="gi">
        <div class="pill"><span class="l">Soru</span><span class="v">${rd}/${mx}</span></div>
        <div class="pill"><span class="l">Süre</span><span class="v" id="bz_tl">${tl}s</span></div>
    ...
    if(tp==='spell'){
      const sub=()=>{const v=document.getElementById('bi').value.trim().toLowerCase();
        if(v===w.word){sc+=30;addScore(30);nx();}else{wrongAnswer();toast('Doğrusu: '+w.word);setTimeout(nx,800);}};
      ...
      document.getElementById('bo').onclick=e=>{const b=e.target.closest('.qopt');if(!b)return;
        ...
        if(b.dataset.w===ca){sc+=30;addScore(30);}else wrongAnswer();
        setTimeout(nx,800);};
    }
  }nx();
}
